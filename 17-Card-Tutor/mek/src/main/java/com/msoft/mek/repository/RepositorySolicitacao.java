...
public interface RepositorySolicitacao extends JpaRepository<ModelSolicitacao, Long> {
    ...
    @Query("select s from ModelSolicitacao s where s.area.areaId = :areaId and lower(s.title) like lower(concat('%', :title, '%'))")
    Page<ModelSolicitacao> findByAreaIdAndTitle(Long areaId, String title, Pageable pageable);

    @Query("select s from ModelSolicitacao s where s.aluno = :aluno and lower(s.title) like lower(concat('%', :title, '%'))")
    Page<ModelSolicitacao> findByAlunoAndTitle(ModelAluno aluno, String title, Pageable pageable);
